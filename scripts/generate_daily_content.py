...
TOPICS = (
    ...
    Topic(
        ...
        "질환과 성분 교육은 설명을 길게 만드는 일이 아니라, 사용자가 이해할 순서와 질문을 설계하는 일입니다. 교육용 플랫폼과 마이크로사이트는 그 흐름을 반복 가능한 자산으로 만듭니다.\n\n출처: bbbb.beauty Company Introduction 2026, 09 / 16",
        "문제:\n전문 정보가 많을수록 학습자는 핵심 경로를 놓치기 쉽습니다.\n\n해결:\n학습자, 상호작용, 의사결정의 순서로 콘텐츠를 설계합니다.\n\n실행:\n한 화면에는 한 질문만 두고, 다음 화면에서 사용자가 직접 확인할 근거를 연결합니다.",
        "교육은 정보 전달이 아니라 이해 가능한 사용자 여정을 만드는 일입니다.",
    ),
    Topic(
        "documentary-trust",
        "Documentary Trust",
    ...
    Topic(
        ...
        "문제:\n제품의 구조와 작용 원리는 정적인 자료만으로 충분히 전달되지 않을 수 있습니다.\n\n해결:\n탐색 대상, 몰입 순간, 사용 환경을 먼저 정합니다.\n\n실행:\n사용자가 한 번의 조작으로 확인해야 할 핵심 정보를 정의한 뒤, 그 정보에 맞는 인터랙션을 설계합니다.",
...
def wrapped(value: str, width: int = 24) -> str:
...
def question_support(topic: Topic, question: str, action: str) -> str:
    return f"{topic.pillar}에서\n{question.replace(chr(10), ' ')}을 먼저 정하고,\n{action}을 한 장면에 남깁니다."
...
def slide(topic: Topic, page: int, eyebrow: str, title: str, body: str) -> Image.Image:
    ...
    eyebrow_width = draw.textbbox((0, 0), eyebrow, font=font(17, True))[2]
    draw.rectangle((58, 166, min(1022, 98 + eyebrow_width), 208), fill=BLACK)
...
def create_package(target_date: date, out_root: Path) -> Path:
    ...
    slides = [
        slide(topic, 1, topic.label, topic.headline, wrapped(topic.instagram)),
        ...
        slide(topic, 5, "SAVE THIS", f"{topic.pillar}\n체크리스트", "세 질문을 먼저 확인한 뒤\n리서치와 제작의 순서를 정리합니다.\n\nSOURCE / bbbb.beauty 2026"),
    ]
