...
결과/교훈: 강한 뷰티 브랜드는 더 예쁘게 포장된 브랜드가 아니라, 더 명확하게 증명되는 브랜드입니다."""
    ...
    x = """뷰티 브랜드의 차별점은 원료 목록이 아니라 고객이 믿을 수 있는 증거 구조입니다."""

    x_thread = """뷰티 브랜드는 자주 원료, 패키지, 트렌드 컬러를 차별점으로 말합니다.

하지만 고객이 실제로 선택하는 순간에는 “왜 믿어도 되는가”가 더 중요합니다.
---
BBBB는 브랜드를 하나의 검증 가능한 가설로 봅니다.

고객 문제, 제품 근거, 감각 신호, 브랜드 언어, 채널 행동이 한 구조로 연결되어야 합니다.
---
Instagram은 보이는 실험, Facebook은 관점 설명, LinkedIn은 실행 프레임, X는 짧은 관점 검증에 맞게 나눕니다.
---
강한 뷰티 브랜드는 더 크게 말하는 브랜드가 아니라 더 쉽게 검증되는 브랜드입니다."""

    x_modes = ["short", "image", "thread", "short", "image"]
    x_mode = x_modes[date.today().toordinal() % len(x_modes)]
    ...
    (OUT / "x-thread.txt").write_text(x_thread, encoding="utf-8")
    (OUT / "x-mode.txt").write_text(x_mode, encoding="utf-8")
    (OUT / "README.md").write_text(
        ...
        f"- X: {x_mode} format in a five-post rotation without default hashtags\n"
